...
public final class PillPreviewView extends View {
    ...
    public void play() { animateTo(shrink, pressMs, false); postDelayed(() -> animateTo(1f, releaseMs, true), pressMs + 120L); }
    private void animateTo(float target, int duration, boolean spring) {
        ValueAnimator a = ValueAnimator.ofFloat(fraction, target);
        a.setDuration(duration);
        if (spring) a.setInterpolator(new OvershootInterpolator(Math.max(.1f, overshoot * 10f)));
    ...
    @Override public boolean onTouchEvent(MotionEvent e) {
        if (e.getActionMasked()==MotionEvent.ACTION_DOWN) { animateTo(shrink,pressMs,false); return true; }
        if (e.getActionMasked()==MotionEvent.ACTION_UP || e.getActionMasked()==MotionEvent.ACTION_CANCEL) { animateTo(1f,releaseMs,true); performClick(); return true; }
        return true;
    }
